#!/usr/bin/env python3
"""
Iteration 052: Prob-Based Dynamic Hold (No Confidence Head)

Instead of using the broken confidence head, use the selected strategy's
probability as hold multiplier:
  - High prob (0.8+) → hold longer (trend likely continues)
  - Low prob (0.4-0.5) → hold shorter (uncertain signal)

hold = base_hold × clip(selected_prob × 2, 0.5, 2.0)

Also: batch_size=4096 (fixed from 051's 16384 disaster).
"""
...
def backtest_modes(model, X_test, kline_15m, kline_4h, strat_info, fee=0.0008, device="cuda"):
    n = len(X_test)
    if n < 100: return {}
    ...
    if T == 0: return {m: {"return": 0.0} for m in ["fixed", "fixed1.5", "fixed2x", "prob_scaled"]}

    # Selected strategy probabilities
    sel_probs = p[np.arange(M)[ht], tj]  # (T,)

    results = {}
    for mode in ["fixed", "fixed1.5", "fixed2x", "prob_scaled"]:
        if mode == "fixed":
            holds = base_holds[tj].copy()
        elif mode == "fixed1.5":
            holds = (base_holds[tj] * 1.5).astype(int)
        elif mode == "fixed2x":
            holds = base_holds[tj] * 2
        else:  # prob_scaled
            # High prob → longer hold
            mults = np.clip(sel_probs * 2, 0.5, 2.0)
        ...
        results[mode] = {
            ...
            "avg_mult": round(float(np.clip(sel_probs * 2, 0.5, 2.0).mean()) if mode == "prob_scaled" else (1.5 if mode == "fixed1.5" else (2.0 if mode == "fixed2x" else 1.0)), 2),
        }

    # Print prob distribution
    print(f"      Prob dist: mean={sel_probs.mean():.3f} std={sel_probs.std():.3f} "
          f"[{np.percentile(sel_probs,10):.2f}, {np.percentile(sel_probs,50):.2f}, {np.percentile(sel_probs,90):.2f}]", flush=True)
...
def main():
    t0 = time.time()
    print("="*60, flush=True)
    print("  ITERATION 052: Prob-Based Dynamic Hold", flush=True)
    print("="*60, flush=True)
    ...
    print(f"\n[2/3] Training (BS=4096)...", flush=True)
    mode_results = {"fixed": [], "fixed1.5": [], "fixed2x": [], "prob_scaled": []}

    for seed in SEEDS:
        sr = {m: [] for m in mode_results}
        for w in range(3):
            ...
            for m in sr: sr[m].append(r[m]["return"])

        for m in mode_results:
            mode_results[m].append(np.mean(sr[m]))

    elapsed = time.time() - t0
    print(f"\n{'='*60}", flush=True)
    print(f"  ITERATION 052 RESULTS", flush=True)
    print(f"{'='*60}", flush=True)
    for m in mode_results:
        ov = np.mean(mode_results[m]); sd = np.std(mode_results[m])
        print(f"  {m:15s}: mean={ov:+.2f}% std={sd:.1f}% seeds={[round(x,1) for x in mode_results[m]]}", flush=True)
    print(f"\n  Time: {elapsed:.0f}s", flush=True)

    report = {"iteration": 52, "approach": "Prob-based dynamic hold (BS=4096)",
              "results": {m: round(np.mean(mode_results[m]), 2) for m in mode_results},
              "time": round(elapsed)}
    with open("reports/iteration_052.json", "w") as f:
