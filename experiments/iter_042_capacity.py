#!/usr/bin/env python3
"""
Iteration 042: Model Capacity Sweep

fusion 128→192 was +7% improvement (iter 041). Push further:
  - expert_hidden: 128 vs 192
  - fusion_dim: 192 vs 256
  - expert_output: 64 vs 96

Using seed 42 only for fast screen, then validate best on 3 seeds.
"""
...
def backtest_vec(model, X_test, kline_15m, kline_4h, strat_info, fee=0.0008, device="cuda"):
    ...
    for i in range(T):
        ...
        pk = max(pk, cap)
    return {"return": round((cap-100000)/1000, 2), "trades": T}


def main():
    t0 = time.time()
    print("=" * 60, flush=True)
    print("  ITERATION 042: Model Capacity Sweep", flush=True)
    print("=" * 60, flush=True)
    ...
    configs = [
        {"name": "e128_o64_f192", "eh": 128, "eo": 64, "fd": 192},   # current best
        {"name": "e192_o64_f192", "eh": 192, "eo": 64, "fd": 192},   # bigger expert
        {"name": "e128_o96_f192", "eh": 128, "eo": 96, "fd": 192},   # bigger expert output
        {"name": "e128_o64_f256", "eh": 128, "eo": 64, "fd": 256},   # bigger fusion
        {"name": "e192_o96_f256", "eh": 192, "eo": 96, "fd": 256},   # all bigger
    ]
    ...
    for cfg in configs:
        ...
        print(f"  {cfg['name']:20s}: mean={overall:+.2f}% std={std:.1f}% params={params:,} seeds={[round(m,1) for m in seed_means]}", flush=True)

    elapsed = time.time() - t0
    print(f"\n  Time: {elapsed:.0f}s", flush=True)

    report = {"iteration": 42, "approach": "Model capacity sweep", "time": round(elapsed)}
    with open("reports/iteration_042.json", "w") as f:
        json.dump(report, f, indent=2, default=str)
    print(f"  Report saved to reports/iteration_042.json", flush=True)
