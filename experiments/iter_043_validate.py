#!/usr/bin/env python3
"""
Iteration 043: Validate Top Configs (Fixed Backtest)

Iter 042 found expert_output=96 gives remarkable stability.
But backtest had a bug (/1000 instead of /100000*100).

Validate top 3 configs with correct backtest:
  1. e128_o96_f192 (iter 042 most stable, std=0.1%)
  2. e192_o64_f192 (iter 042 highest return)
  3. e128_o64_f192 (iter 041 baseline that got +47%)
"""
...
def train_rdrop(model, train_ds, val_ds, epochs=50, batch_size=2048,
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            total = (l1["total"] + l2["total"]) / 2 + 1.0 * _kl_binary(
...
def backtest_vec(model, X_test, kline_15m, kline_4h, strat_info, fee=0.0008, device="cuda"):
    """Vectorized backtest — FIXED return calculation."""
    n = len(X_test)
    if n < 100:
        return {"return": 0.0, "trades": 0, "wr": 0.0, "bh": 0.0, "n_long": 0, "n_short": 0}
    ...
    valid = (p >= thresh) & (ev > 0)
    ev_m = np.where(valid, ev, -np.inf)
    bj = np.argmax(ev_m, axis=1)
    ht = ev_m[np.arange(M), bj] > 0
    ti, tj = idx[ht], bj[ht]; T = len(ti)
    if T == 0:
        bh = (tc[-1] - tc[0]) / tc[0] * 100 if tc[0] > 0 else 0
        return {"return": 0.0, "bh": round(bh, 2), "trades": 0, "wr": 0.0, "n_long": 0, "n_short": 0}
    ei = np.minimum(ti + holds[tj], n - 1)
    net = dirs[tj] * (tc[ei] - tc[ti]) / tc[ti] - fee
    ...
    return {"return": round(ret, 2), "bh": round(bh, 2), "trades": T,
            ...
            "n_long": int((dirs[tj] > 0).sum()), "n_short": int((dirs[tj] < 0).sum())}


def main():
    t0 = time.time()
    print("=" * 60, flush=True)
    print("  ITERATION 043: Validate Top Configs (Fixed Backtest)", flush=True)
    print("=" * 60, flush=True)
    ...
    configs = [
        {"name": "e128_o64_f192", "eh": 128, "eo": 64, "fd": 192},
        {"name": "e128_o96_f192", "eh": 128, "eo": 96, "fd": 192},
        {"name": "e192_o64_f192", "eh": 192, "eo": 64, "fd": 192},
    ]

    print(f"\n[2/3] Validating {len(configs)} configs × {len(SEEDS)} seeds × 3 windows...", flush=True)
    all_results = {}
        ...
        for seed in SEEDS:
            ...
            for w in range(3):
                ...
                model = PLEv4(feature_partitions=partitions, n_account_features=4, n_strategies=ns,
                              expert_hidden=cfg["eh"], expert_output=cfg["eo"], fusion_dim=cfg["fd"], dropout=0.2)
            ...
            seed_rets = [r["return"] for r in wrets]
            seed_results[seed] = {"windows": seed_rets, "mean": round(np.mean(seed_rets), 2), "details": wrets}
            print(f"    {cfg['name']} seed {seed}: {[round(x, 1) for x in seed_rets]} → mean={np.mean(seed_rets):+.2f}%", flush=True)

        seed_means = [seed_results[s]["mean"] for s in SEEDS]
        overall = np.mean(seed_means)
        std = np.std(seed_means)
        params = sum(p.numel() for p in model.parameters())
        all_results[cfg["name"]] = {
            "config": cfg, "seeds": seed_results,
            "overall": round(overall, 2), "std": round(std, 2), "params": params
        }
        print(f"  {cfg['name']:20s}: mean={overall:+.2f}% std={std:.1f}% params={params:,}", flush=True)

    elapsed = time.time() - t0
    best = max(all_results.items(), key=lambda x: x[1]["overall"])

    print(f"\n{'=' * 60}", flush=True)
    print(f"  ITERATION 043 RESULTS", flush=True)
    print(f"{'=' * 60}", flush=True)
    for name, r in all_results.items():
        sm = [r["seeds"][s]["mean"] for s in SEEDS]
        print(f"  {name:20s}: mean={r['overall']:+.2f}% std={r['std']:.1f}% seeds={sm}", flush=True)
    print(f"\n  BEST: {best[0]} (mean={best[1]['overall']:+.2f}%, std={best[1]['std']:.1f}%)", flush=True)
    print(f"  Time: {elapsed:.0f}s", flush=True)

    report = {
        "iteration": 43,
        "approach": "Validate top configs with fixed backtest",
        "results": {k: {"overall": v["overall"], "std": v["std"], "params": v["params"],
                         "seed_means": {str(s): v["seeds"][s]["mean"] for s in SEEDS}}
                     for k, v in all_results.items()},
        "best": best[0],
        "mean": best[1]["overall"],
        "time_seconds": round(elapsed),
    }
    with open("reports/iteration_043.json", "w") as f:
        json.dump(report, f, indent=2, default=str)
    print(f"\n  Report saved to reports/iteration_043.json", flush=True)
