#!/usr/bin/env python3
"""
Iteration 039: Regime Expert Partition + High-Confidence Scalp

Two improvements:
  1. Regime Expert Partition (해상도를 올리면서 레짐 파악):
...
def main():
    t0 = time.time()
    print("=" * 60, flush=True)
    print("  ITERATION 040: Regime Expert Partition", flush=True)
    print("=" * 60, flush=True)
    ...
    print(f"\n{'=' * 60}", flush=True)
    print(f"  ITERATION 040 RESULTS", flush=True)
    ...
    with open("reports/iteration_040.json", "w") as f:
        json.dump(report, f, indent=2, default=str)
    print(f"\n  Report saved to reports/iteration_040.json", flush=True)
